...
* **Terraform** – AWS infrastructure provisioning (VPC, subnets, security groups, EC2)
* **Ansible** – Server configuration (packages, Nginx, Docker, web content)
* **Jenkins** – CI/CD automation using Pipeline as Code (`Jenkinsfile`)
...
3. Jenkins runs Terraform:
   ...
   * `init` → `plan` → **manual approval** → `apply`
...
4. **Manual Approval** – Safety gate before applying changes
5. **Terraform Apply** – Provision infrastructure
6. **Wait for EC2** – Allow instances to finish booting
7. **Debug Inventory** – Optional diagnostics
8. **Configure with Ansible** – Run playbooks on provisioned hosts
...
Feel free to extend the playbooks, Terraform modules, and Jenkins stages to fit more advanced use cases 🚀
